...
class Multi_head_attention_trs(nn.Module):
    ...
    def __init__(self, hidden_size, nhead=8, dropout=0.3):
        ...
        if hidden_size % nhead != 0:
            raise Exception('hidden_size must be divisble by nhead, but got {hidden_size}/{nhead}.'.format(hidden_size, nhead))
...
try:
    ...
    print('[!] cannot find the module "torch_geometric", ignore it')